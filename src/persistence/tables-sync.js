...
import { assembleFromNormalizedRows, defaultState, buildRubric } from '../model/rubric.js';
...
// ---- per-assessment selections (priorities / sub-needs / captured use cases) ----
// These live in their own tables now instead of one assessments.data jsonb
// blob, so a priority click and a use-case rename never touch the same row.
export async function loadAssessmentSelections(assessmentId) {
  const [priorities, subNeeds, useCases, ucCapLinks] = await Promise.all([
  ...
  const moscow = { ...base.moscow }; priorities.forEach(r => { moscow[r.capability_id] = r.moscow; });
  const needs = { ...base.needs }; subNeeds.forEach(r => { needs[r.sub_capability_id] = !!r.needed; });
  ...
  const useCasesArr = useCases.map(u => ({
    id: u.id, title: u.title, desc: u.description,
    capIds: capsByUC.get(u.id) || [], sourceLibId: u.source_lib_id || null,
  }));
  store.S = { moscow, needs, useCases: useCasesArr };
  store.collapsedPillars.clear();
  requestRender();
}

// Realtime for one assessment's own tables. Lower collision risk than the
// shared rubric (typically one consultant runs one client engagement at a
// time), so this always reapplies on change rather than tracking per-row
// dirty state — simpler, and safe for the traffic this sees in practice.
export function subscribeAssessment(assessmentId, { onLoaded } = {}) {
  ...
  const channel = sb.channel('assessment-' + assessmentId + '-sync')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'assessment_priorities', filter: 'assessment_id=eq.' + assessmentId }, reapply)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'assessment_sub_needs', filter: 'assessment_id=eq.' + assessmentId }, reapply)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'assessment_use_cases', filter: 'assessment_id=eq.' + assessmentId }, reapply)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'assessment_use_case_capabilities', filter: 'assessment_id=eq.' + assessmentId }, reapply)
    .subscribe();
  return channel;
}
